...
@section('main')
@include('layout.page-title', ['name' => 'Nhân viên','tree' => 2])
                    ...
                    <div class="col">
                        <select name="" multiple="multiple" id="group">
                            ...
                            @foreach($list_subject as $v)
                                <option value="{{$v->id}}">{{$v->name}} - {{$v->department}}</option>
                            @endforeach
                            @endif
                        </select>
                    </div>
...
<script>
    ...
    if (typeof param[1] !== 'undefined') {
        ...
        $.ajax({
            ...
            //console.log(op_se);
            $.each(option, (i,v) => {
                var sj_id = $(v).attr('value');
                $.each(result, (k,result_value) =>{
                    if(result_value.id == sj_id){
                        $(v).attr("selected", true);
                    }
                });
            });
            $.each(op_se, (i,v) => {
                var app_id = $(v).attr('data-value');
                //console.log(app_id);
                $.each(result, (k, result_value) =>{
                    if(result_value.id == app_id){
                        console.log('!!!');
